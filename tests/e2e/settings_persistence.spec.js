...
test.describe('Settings Persistence E2E', () => {
    ...
    test.beforeEach(async ({ page }) => {
        ...
        const responsePromise = page.waitForResponse(r => r.url().includes('/api/settings') && r.method() === 'GET', { timeout: 30000 }).catch(() => null);
        await page.goto('/');
    ...
    test('should persist all settings after reload', async ({ page }) => {
        ...
        for (const [key, values] of Object.entries(testData)) {
            ...
            const savePromise = page.waitForResponse(r => r.url().includes('/api/settings') && r.method() === 'POST', { timeout: 30000 });
        ...
        const humanEmulationLabel = page.locator('label:has-text("Эмуляция человека"), label:has-text("Human Emulation")');
        const concurrentProfilesInput = page.locator('label:has-text("Профилей:"), label:has-text("Profiles:"), label:has-text("Concurrent Profiles")').locator('input[type="number"]');
        ...
        const saveShowBrowser = page.waitForResponse(r => r.url().includes('/api/settings') && r.method() === 'POST', { timeout: 30000 });
        await showBrowserLabel.click();
        await saveShowBrowser;

        console.log(`Toggling Human Emulation (initial: ${initialHumanEmulation})...`);
        const saveHumanEmulation = page.waitForResponse(r => r.url().includes('/api/settings') && r.method() === 'POST', { timeout: 30000 });
        ...
        const saveConcurrent = page.waitForResponse(r => r.url().includes('/api/settings') && r.method() === 'POST', { timeout: 30000 });
        ...
        await expect(humanEmulationLabel.locator('input')).toBeChecked({ checked: !initialHumanEmulation });
