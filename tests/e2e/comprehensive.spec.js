...
test.describe('IG-Bot Comprehensive E2E', () => {
  ...
  test('should navigate between tabs', async ({ page }) => {
    ...
    await expect(
      page.locator('h3:has-text("Фарм профилей"), h3:has-text("Profile Scraper")')
    ).toBeVisible();
    await page.click('button:has-text("Настройки"), button:has-text("Settings")');
    await expect(
      page.locator('button:has-text("Аккаунты"), button:has-text("Accounts")')
    ).toBeVisible();
  });
  ...
  test('should manage accounts and settings', async ({ page }) => {
    ...
    await page.click('button:has-text("Управление"), button:has-text("Execution")');
    const showBrowserSwitch = page
      .locator('label:has-text("Показывать браузер"), label:has-text("Show browser")')
  ...
  test('should interact with profiles', async ({ page }) => {
    ...
    const emptyMessage = page.locator(
      'text=Нет профилей по выбранным фильтрам, text=No profiles found'
    );
