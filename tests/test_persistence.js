...
async function testPersistence() {
    ...
    try {
        ...
        await db.run(
            ...
            [testAccount.id, testAccount.name, testAccount.proxy, testAccount.cookies, testAccount.fingerprint]
        );
        ...
        for (const kw of testKeywords) {
            const list = await getList(kw.type === 'city' ? 'cityKeywords.txt' : (kw.type === 'name' ? 'names.txt' : 'nicheKeywords.txt'));
        ...
        console.log('Note: Since we use the same process and a singleton DB instance, we just verify the data still exists.');
