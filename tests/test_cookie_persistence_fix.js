...
async function testCookiePersistence() {
    ...
    await db.run('INSERT INTO accounts (id, name, cookies) VALUES (?, ?, ?)', [testId, 'Test Account', originalCookies]);
    ...
    async function mockSettingsUpdate(incomingAccount) {
        ...
        if (!finalCookies) {
            const existing = await db.get('SELECT cookies FROM accounts WHERE id = ?', [incomingAccount.id]);
            if (existing && existing.cookies) {
                finalCookies = existing.cookies;
            }
        }
        await db.run('UPDATE accounts SET cookies = ? WHERE id = ?', [finalCookies, incomingAccount.id]);
    }
