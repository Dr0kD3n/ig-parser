...
const extractVisibleCandidates = () => {
    ...
    canvases.forEach(canvas => {
        ...
        if (!storyBtn || storyBtn.getAttribute('aria-disabled') === 'true')
            return;
...
const analyzeProfile = async (context, url, config, donor = '') => {
    ...
    try {
        ...
        await (0, reporter_1.saveCrashReport)(page, e, `analyze_profile_${url.split('/').filter(Boolean).pop()}`);
    }
    finally {
        await page.close();
    }
...
const processDonor = async (context, donorUrl, config, totalAccounts = 0) => {
    ...
    try {
        ...
        await (0, reporter_1.saveCrashReport)(page, e, `donor_${donorUrl.split('/').filter(Boolean).pop()}`);
        throw e;
    }
    finally {
        await page.close();
