...
const refreshProfile = async (context, profile, config) => {
  ...
  try {
    ...
    const data = await page
      ...
      .catch(() => ({ pUrl: '', fCount: 0, postCount: 0, bio: '', name: username }));
    ...
    if (finalFollowers && finalFollowers !== (existing?.followers_count || 0))
      updated.push('подписчики');
    if (finalPosts && finalPosts !== (existing?.publications_count || 0))
      updated.push('публикации');
...
const run = async () => {
  ...
  const noPhoto = await db.get(
    `SELECT COUNT(*) as cnt FROM profiles WHERE photo IS NULL OR photo = ''`
  );
  const noFollowers = await db.get(
    `SELECT COUNT(*) as cnt FROM profiles WHERE followers_count IS NULL OR followers_count = 0`
  );
  ...
  for (let i = 0; i < profiles.length; i += CHUNK_SIZE) {
    ...
    logger.info(
      `📦 Пачка ${Math.floor(i / CHUNK_SIZE) + 1} / ${Math.ceil(profiles.length / CHUNK_SIZE)}`
    );
