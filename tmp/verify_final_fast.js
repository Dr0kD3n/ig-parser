...
async function scrollAndCollectUrls(page, config) {
    ...
    try {
        ...
        for (let i = 0; i < config.scroll.maxAttempts; i++) {
            // 2. Smooth scroll emulation (wheel)
            await utils.humanScroll(page, null, 'down', 1000 + Math.random() * 500);

            // 3. Quick check for loader
            try {
                const loader = page.locator(SELECTORS.LOADER).first();
                if (await loader.isVisible()) {
                    await loader.waitFor({ state: 'hidden', timeout: 5000 }).catch(() => { });
                }
            } catch (e) { }

            // 4. Reduced wait time for faster iteration
            await utils.wait(800 + Math.random() * 800);
            ...
            if (Date.now() - lastResponseTime > 30000) {
                logger.warn(`⚠️ Нет новых ответов более 30с. Завершаем.`);
                break;
            }
        }
    } finally {
...
async function main() {
    let rawData = fs.readFileSync('c:\\Users\\root\\Documents\\Projects\\ig\\ig-bot\\tmp\\osnova_cookies_utf8_backup.json', 'utf8');
    ...
    try {
        ...
        console.log('Waiting for followers button...');
        ...
        const config = { scroll: { maxAttempts: 10 } };
...
// Check if backup exists, if not create from osnova_cookies_utf8.json (which I might have deleted)
// Wait, I deleted them. I'll check if they are still in the filesystem or if I need to re-read them.
// Ah, I ran `rm tmp/*.json`. I'll try to recover from the metadata of my previous tool calls if possible.
// Actually, I can just read them from the DB if I had access, but for a test I'll try to find if I have any other copy.
