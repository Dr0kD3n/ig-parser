...
export default function SettingsTab({ settingsData, onSettingsChange, tr, isLoading, authFetch, failedUrls }) {
    ...
    const handleOpenBrowser = async (id, forceRestore = false) => {
        try {
            ...
            const data = await res.json();
            if (data.success)
                toast.success(forceRestore ? tr('browser_restore_success') || 'Browser opened with auto-photo-load' : tr('browser_success'));
            else
                toast.error(data.error);
        }
        catch (e) {
    ...
    const handleWarmup = async (id) => {
        try {
            ...
            const data = await res.json();
            if (data.success)
                toast.success(tr('warmup_started'));
            else
                toast.error(data.error);
        }
        catch (e) {
    ...
    const renderTaskSection = (field, label) => {
        ...
        return (<div className="task-section-card">
            <h4 className="task-section-title">{label}</h4>
            <div className="flex-v gap-8">
                {activeAccounts.length === 0 && <div className="no-accs-placeholder">{tr('no_accounts_selected')}</div>}
                {activeAccounts.map((acc, idx) => (<div key={acc.id} draggable onDragStart={(e) => onDragStart(e, idx, field)} onDragOver={(e) => onDragOver(e, idx, field)} className="active-acc-item" onMouseEnter={(e) => e.currentTarget.classList.add('border-primary')} onMouseLeave={(e) => e.currentTarget.classList.remove('border-primary')}>
                    <span className="drag-handle">☰</span>
                    <span className="acc-name-label">{acc.name}</span>
                    <button onClick={() => toggleAccountForTask(field, acc.id)} className="acc-remove-btn">
                        ✕
                    </button>
                </div>))}
            </div>
        ...
        <div className="settings-header">
            ...
            <div className="header-right gap-20">
                ...
                <label className="checkbox-label">
                    ...
                    <input type="number" min="1" max="20" value={settingsData.concurrentProfiles || 3} className="num-input-sm" onChange={e => onSettingsChange({ ...settingsData, concurrentProfiles: parseInt(e.target.value) || 1 })} />
                </label>
                {updateInfo?.hasUpdate && (
                    ...
                    >
                        {isUpdating ? tr('checking_updates') : tr('update_available').replace('{version}', updateInfo.latestVersion)}
                    </button>
                )}
            </div>
        </div>

        {settingsTab === 'accounts' && (<div className="settings-grid">
                    ...
                    {settingsData.accounts.map(acc => (<div key={acc.id} className="account-card">
                        {editingAccount === acc.id ? (<div className="flex-v gap-10">
                            ...
                            <div className="flex gap-8">
                                ...
                                <button className="btn-primary btn-outline btn-sm btn-ghost flex-1 color-muted" onClick={() => setEditingAccount(null)}>
                                    {tr('cancel')}
                                </button>
                            </div>
                        </div>) : (<>
                            <div className="flex-between mb-4">
                                <div className="flex-1 flex-baseline gap-8">
                                    ...
                                    {acc.warmup_score > 0 && (<div className="acc-card-score">
                                        {acc.warmup_score}%
                                    </div>)}
                                </div>

                                <button className="editBtn" onClick={() => handleStartEdit(acc)} title={tr('edit_title')}>
                                    <EditIcon />
                                </button>
                                <button className="deleteBtn" onClick={() => handleDelete(acc.id)} title={tr('delete_title')}>
                                    <TrashIcon />
                                </button>
                            </div>
                            <div className="acc-proxy-text">
                            ...
                            <div className="acc-action-bar">
                                <button onClick={() => handleLogin(acc.id)} className="btn-acc-action btn-acc-login">
                                    {tr('btn_login')}
                                </button>
                                <button onClick={() => handleOpenBrowser(acc.id, true)} className="btn-acc-action btn-acc-browser">
                                    {tr('btn_open_browser')}
                                </button>
                                <button onClick={() => handleWarmup(acc.id)} className="btn-acc-action btn-acc-warmup">
                                    {tr('btn_warmup')}
                                </button>
                            </div>
                        </>)}
