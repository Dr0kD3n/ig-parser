...
const LogGroup = memo(function LogGroup({ group, tr }) {
    ...
    return (<div className={`log-group${collapsed ? ' collapsed' : ''}`}>
        ...
        {!collapsed && (<div className="log-group-content">
            {group.logs.map((log, i) => (<div className="log-entry" key={log.id || i}>
...
export default function ControlsTab({ botStatus, onBotControl, onClearLogs, logs, tr, isLoading, token }) {
    ...
    return (<div className="controls-panel tab-content-fade">
        ...
        <div className="control-card">
            ...
            {!scraperCollapsed && (<div className="flex-v gap-12 mt-12">
                <p className="control-desc">{tr('scraper_desc')}</p>
                {botStatus.index
                    ? (<div className="flex gap-12">
                        <button className="btn-primary btn-danger btn-ghost" onClick={() => onBotControl('index', 'stop')}>{tr('btn_stop_scraper')}</button>
                        <button className="btn-primary btn-warning btn-ghost" onClick={async (e) => {
        ...
        <div className="control-card">
            ...
            {!parserCollapsed && (<div className="flex-v gap-12 mt-12">
                <p className="control-desc">{tr('parser_desc')}</p>
                {botStatus.parser
                    ? <button className="btn-primary btn-danger btn-ghost" onClick={() => onBotControl('parser', 'stop')}>{tr('btn_stop_parser')}</button>
        ...
        <div className={`control-card logs-card${logsCollapsed ? ' collapsed' : ''}`}>
            <div className="logs-header" style={{ marginBottom: logsCollapsed ? 0 : 12 }}>
                ...
                <div className="control-status-group">
                    <button className="btn-primary btn-ghost btn-sm btn-outline btn-clear" onClick={(e) => {
                        e.stopPropagation();
                        onClearLogs();
                    }}>
                        {tr('btn_clear')}
                    ...
                    <span className="count-badge cursor-pointer" onClick={toggleLogs}>{logsCollapsed ? '▼' : '▲'}</span>
                </div>
            </div>
            {!logsCollapsed && (<div ref={logBoxRef} id="log-box" className="logs-container">
                {groups.length === 0 && (<div style={{ color: 'hsl(var(--text-dim))', textAlign: 'center', padding: '40px' }}>{tr('no_logs')}</div>)}
                {groups.map((group, idx) => (<LogGroup key={group.sessionId + group.source + idx} group={group} tr={tr} />))}
        ...
        <div className={`stream-card${streamCollapsed ? ' collapsed' : ''}`}>
            ...
            {!streamCollapsed && (<div className="stream-container" onClick={() => setIsZoomed(true)}>
                <img src={`/api/live-view?t=${liveViewTimestamp}&token=${token}`} className={(botStatus.index || botStatus.parser) ? 'block' : 'hidden'} alt="Live View" onError={(e) => {
                    e.target.style.display = 'none'; if (e.target.nextSibling)
                        e.target.nextSibling.style.display = 'block';
                }} onLoad={(e) => {
                    e.target.style.display = 'block'; if (e.target.nextSibling)
