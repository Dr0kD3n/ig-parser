      ...
      <div className="log-group-header" onClick={() => setCollapsed((c) => !c)}>
        <div className="group-info">
          ...
          <span className="group-label">
            {tr('batch_entries').replace('{count}', String(group.logs.length))}
          </span>
        </div>
        ...
        {!scraperCollapsed && (
          <div className="flex-v gap-12 mt-12">
            ...
            {botStatus.index ? (
              ...
              <button className="btn-primary" onClick={() => onBotControl('index', 'start')}>
                {tr('btn_start_scraper')}
              </button>
            )}
          </div>
        )}
        ...
        {!parserCollapsed && (
          <div className="flex-v gap-12 mt-12">
            ...
            {botStatus.parser ? (
              ...
              <button className="btn-primary" onClick={() => onBotControl('parser', 'start')}>
                {tr('btn_start_parser')}
              </button>
            )}
          </div>
      ...
      <div className={`stream-card${streamCollapsed ? ' collapsed' : ''}`}>
        ...
        {!streamCollapsed && (
          <div className="stream-container" onClick={() => setIsZoomed(true)}>
            <img
              src={`/api/live-view?t=${liveViewTimestamp}&token=${token}`}
              className={botStatus.index || botStatus.parser ? 'block' : 'hidden'}
              alt="Live View"
              onError={(e) => {
