...
try {
  ...
  execSync('npx pkg . --targets node18-win-x64 --no-bytecode --output ../dist_package/ig-bot.exe', {
    cwd: backendDir,
    stdio: 'inherit',
  });
  ...
  fs.writeFileSync(
    path.join(distDir, 'install_browsers.bat'),
    '@echo off\necho Installing Chromium for Playwright...\nnpx playwright install chromium\npause'
  );
  fs.writeFileSync(
    path.join(distDir, 'kill_servers.bat'),
